...
| 8.4 ▲ | Fee schedule + booking tagging (§16.6) | ✅ **BACKEND DONE** — migration `Phase8FeeScheduleAndVisitType`: fee cols on `clinic_settings` (450/350/400/50, `discount_pct` 0.20), `bookings.visit_type` (`New`/`FollowUp`, string enum), `bookings.discount_category`/`discount_amount`/`med_cert_requested`, `patients.senior_id_number`/`pwd_id_number`. `PUT /api/settings` persists the fee schedule. `PUT /api/consultations/by-booking` accepts `visit_type` / `med_cert_requested` / `discount_category` and **recomputes `total_fee`/`amount_due`/`consultation_fee_snapshot` server-side** on the linked booking (flat-line: senior/PWD 400 → else follow-up 350 → else 450, `+ 50` med-cert). Data layer: `ClinicSettingsRow` + `ConsultationUpsert` carry the new fields. ⏳ **FE screens pending** — admin/settings fee editor, provisional fee at reservation, doctor's visit-type + med-cert toggles on the consultation page, secretary collect-against-final |
