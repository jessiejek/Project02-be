...
| 8.2 ▲ | Server-side pagination + search (§16.2) | ▲ **PATTERN LANDED (patients)** — `GET /api/patients/search?q=&page=&pageSize=&sort=` returns `PagedResult<Patient>` (`items`/`totalCount`/`page`/`pageSize`, offset paging via `Skip/Take`, `sort` = name\|code\|created with `-` prefix). Kept the plain `GET /api/patients` for typeahead callers. Smoke-verified (page/size/sort/q). **Offset paging, not keyset** — fine at this data scale; revisit if a table grows large. | ✅ `queryPatientsPaged` + `PagedResult<T>` in `patients.ts` (supabase-mode falls back to a client slice); `admin/patients` rebuilt with debounced server search + prev/next pager + "X–Y of N". ⏳ **Remaining** — apply the same to doctors / staff / bookings / audit-logs list screens; `doctor/patients` card grid → `DataTable` |
